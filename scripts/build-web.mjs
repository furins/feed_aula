...
const historySource = join(rootDir, 'history.html');
...
async function build() {
  ...
  const [desktopHtml, scanHtml, configHtml, identitiesHtml, historyHtml] = await Promise.all([
    readFile(desktopSource, 'utf8'),
    readFile(scanSource, 'utf8'),
    readFile(configSource, 'utf8'),
    readFile(identitiesSource, 'utf8'),
    readFile(historySource, 'utf8')
  ]);
  ...
  await Promise.all([
    ...
    writeFile(join(distDir, 'history.html'), historyHtml, 'utf8'),
